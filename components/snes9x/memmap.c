...
void S9xDeinterleaveType1(int32_t TotalFileSize, uint8_t* base)
{
   ...
   tmp = (uint8_t*) heap_caps_malloc(0x8000, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
...
void S9xDeinterleaveGD24(int32_t TotalFileSize, uint8_t* base)
{
   ...
   /* DS2 DMA notes: base may or may not be 32-byte aligned */
   tmp = (uint8_t*) heap_caps_malloc(0x80000, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
...
static char* Safe(const char* s)
{
   ...
   if (!safe || len + 1 > safe_len)
   {
      if (safe)
         free(safe);
      safe = (char*) heap_caps_malloc(safe_len = len + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
   }
...
uint8_t TileCached[MAX_2BIT_TILES];
#ifdef DISABLE_TILE_CACHE
uint8_t TileCache[MAX_2BIT_TILES];
#endif

bool S9xInitMemory(void)
{

   Memory.RAM   = (uint8_t*) heap_caps_malloc(RAM_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);  //128 KB
   Memory.SRAM  = (uint8_t*) heap_caps_malloc(SRAM_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);  // 128 KB
   Memory.VRAM  = (uint8_t*) heap_caps_malloc(VRAM_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);  // 64 KB
   Memory.FillRAM = (uint8_t*) heap_caps_malloc(0x8000, MALLOC_CAP_INTERNAL);  // 32 KB

#ifdef DISABLE_TILE_CACHE
   IPPU.TileCache = (uint8_t*) TileCache;// 4kB
#else
   IPPU.TileCache = (uint8_t*) heap_caps_malloc(MAX_2BIT_TILES * 128, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);  // 512kB
#endif

   IPPU.TileCached = (uint8_t*) heap_caps_malloc(MAX_2BIT_TILES, MALLOC_CAP_INTERNAL);  // 4KB
   bytes0x2000 = (uint8_t *) heap_caps_malloc(0x2000, MALLOC_CAP_INTERNAL);  // 8 KB

   Memory.PSRAM   = (uint8_t*) heap_caps_malloc(MAX_ROM_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);   // 5MB  // This is where the ROM goes if loaded to PSRAM

   if (!Memory.RAM || !Memory.VRAM || !Memory.SRAM || !Memory.FillRAM || !Memory.PSRAM
   ...
   memset((void*) Memory.SRAM, 0x00, SRAM_SIZE);
...
again:
   ...
   {
      printf("WARNING: ROM size exceeds predefined max (%d)!\n", TotalFileSize);
...
void S9xDeinterleaveType2(bool reset)
{
   if (Memory.ROM_is_writable) {
      ...
      tmp = (uint8_t*) heap_caps_malloc(0x10000, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
